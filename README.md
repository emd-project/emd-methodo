...
les garde-fous anti-IA et la génération d'images. Installé **une fois** par machine, mis à
jour **au centre** (un push ici → tout le monde fait `/plugin update`).
...
| Type | Détail |
|---|---|
| **Skills** | `nouveau-site` (routeur d'init), `init-site`, `configure-from-spec`, `seo-geo-redaction`, `integrate-claude-design`, `ton-of-voice`, `humaniser-fr` |
...
La génération d'images passe par `mcp__nano-mentionbox__generate_image`, un serveur **local**
dont le chemin dépend de l'emplacement de l'app sur chaque machine. Il n'est donc PAS bundlé
ici : il reste câblé par l'installeur de l'app Nano MentionBox (`Installer MCP - …`).

L'installeur de l'app peut être étendu pour ajouter aussi ce plugin → **install groupée**
(MCP + plugin en une action), tout en gardant des **mises à jour séparées** (app via `git pull`,
méthodo via `/plugin update`).
