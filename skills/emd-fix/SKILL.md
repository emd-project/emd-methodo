...
- **Identité** : favicon manquant/générique → générer un favicon UNIQUE (1:1) **et le câbler via `app/icon.svg`** ; logo manquant/générique/cloné → générer/dessiner une **marque unique** et la poser **là où elle est rendue** (SVG inline du `Nav.tsx`/`Header.tsx`), pas juste `logo.svg`. Vérifier le rendu (cf. §0).
...
- DA : hex en dur → tokens ; contraste — sans changer l'identité unique du site.
...
`pipeline/fixes/fix-AAAA-MM-JJ.md` dans `emd-project/emd-methodo` : par site, corrigé vs **reporté** (plafond, risqué, home type à revoir) vs **à vérifier** (câblage non prouvé). N'écrase PAS l'audit.

## 4 — Rapport de run
Sites touchés, correctifs mécaniques (dont RGPD/responsive/identité/auteur/GEO), opérations lourdes faites vs reportées, **assets dont le câblage reste à confirmer**, problèmes restants.
...
- FR + EN (jamais NL) ; contenu/featured 16:9, favicon/logo/avatar 1:1, featured unique + 2 images in-body ; **auteur persona unique (jamais « la rédaction »)** ; **GEO : ≥ 70 % H2-questions + réponse-first + FAQ 6-7** ; bandeau cookies RGPD monté ; 0 scroll horizontal ; texte via `humaniser-fr` ; DA unique préservée.
