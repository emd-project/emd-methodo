...
Tout converge vers **un seul chemin de configuration : `configure-from-spec`** (alimenté par un `init-spec.md`). C'est le même chemin que la provision autonome (`emd-provision-sites`), donc un seul codepath à maintenir.
...
- **Indisponible** → STOP. Prévenir l'utilisateur :
  ...
  Ne pas continuer en mode dégradé silencieux. L'init produit OBLIGATOIREMENT une DA + des images (cf. skill `configure-from-spec`).
...
1. **`init-spec.md` à la racine ?** (`ls init-spec.md` ou Glob)
   - **Oui** → entrée wizard / spec fournie. Router vers **`configure-from-spec`**. Annoncer :
     > `init-spec.md` détecté. Je configure le site automatiquement depuis la spec.
...
3. **Repo frais, pas de spec** → **générer un `init-spec.md` en mode fullAuto** (Claude décide DA/niche/auteur/contenu à partir du nom de domaine et du secteur, comme la provision), PUIS router vers **`configure-from-spec`**. Annoncer :
   > Pas de `init-spec.md` : je génère une spec fullAuto (DA + niche + auteur déduits du domaine), puis je configure le site.
   - **Astuce humain** : pour guider les choix plutôt que tout laisser en auto, dépose un `init-spec.md` à la racine AVANT de lancer (le point 1 le prendra).

## Étape 2 — Déléguer

Charger **`configure-from-spec`** et le laisser dérouler sa procédure complète (DA composée, identité câblée, auteur persona, légal, cookies, seed, images). Ce routeur ne réécrit pas sa logique — il prépare la spec puis l'exécute.

Rappel non négociable : un site qui sort de l'init avec le **thème par défaut** (`#FF3D57`, dark/aurora, logo `emd·template`) OU avec des **placeholders d'images** est un **échec d'init**. La DA composée + les images structurelles sont obligatoires. Respecter `references/garde-fous.md` (build-risqué = signaler, jamais écraser du contenu par du vide).
