...
- Pas de `noindex/nofollow` accidentel : les articles FR et EN sortent en `index, follow` (root metadata `robots: index/follow`).
...
- **OG réellement câblé** : `app/opengraph-image.tsx` (edge, `ImageResponse`, palette du site) + OG par article dans `generateMetadata` (title/desc/url/type article/publishedTime/author/image). `twitter:card=summary_large_image` présent.
- **JSON-LD** servi sur les articles FR et EN : `BreadcrumbList` + `Article` (author `Person` + `speakable`) + `FAQPage`.
- Slugs propres et naturels par langue (`simulateur-assurance-auto-belgique` / `simulate-car-insurance-belgium`). Aucun lien mort détecté sur les pages sondées (les `ERR_ABORTED` observés sont des prefetch RSC annulés, sans impact).

**Multilingue réel (critères `references/i18n-multilingue.md`)**
1. Arbre `app/en/` séparé et complet (layout, page, blog, legal-notice, privacy, catégories, outils) — ne touche pas aux routes FR. ✅
2. Lecteurs miroir `getAllArticlesEn()` / `getCategoriesEn()` / `getArticleRawEn()` / `articleExistsEn()` dans `lib/blog.ts`. ✅
3. **Dossiers de locale jamais lus comme catégories** : `RESERVED_LOCALE_DIRS` (`en`/`nl`/`de`) + liste blanche `FR_CATEGORY_WHITELIST` issue de `niche.categories`. ✅ → **pas de pseudo-multilingue**, pas de fausses catégories.
4. Mapping FR↔EN dans `lib/i18n/article-slugs.ts` (+ `articleSlugInOrNull`, fallback home, jamais de 404). ✅ (incomplet, cf. point 5)
5. `LangSwitch` **importé ET rendu** dans `Nav.tsx` (desktop `nav-cta` + menu mobile), sous `isMultilingual()`, locale déduite du path. ✅
6. hreflang réciproque + x-default. ✅ — attribut `lang` : ⚠️ (point 2)
7. **Parité de contenu** : 1 article réel FR ↔ 1 article réel EN (`comparatifs`), + le doublon de démo `guides`. Aucun orphelin de traduction. ✅
8. **Aucun NL** nulle part (`locales: ['fr','en']`). ✅

**Santé**
- `/` → 200 (title + meta description OK, CLS 0, load ~0,5 s).
- `/en` → 200 (chrome EN, contenu EN).
- `/blog/comparatifs/simulateur-assurance-auto-belgique` → 200 ; `/en/blog/comparatifs/simulate-car-insurance-belgium` → 200.
- ⚠️ `https://www.simulateur-assurance-auto.be` → **connexion refusée** (domaine non branché).

## Top 5 des actions prioritaires

1. **Brancher le domaine `simulateur-assurance-auto.be` sur Vercel** (ou fixer `NEXT_PUBLIC_SITE_URL` sur l'alias vercel.app) — sans ça, canonicals + hreflang + OG pointent tous vers un host mort.
2. **Poser `lang="en"` sur le `<html>` des routes `/en`** (root layout locale-aware, ou `generateMetadata`/segment layout qui remonte l'attribut) — critère 6 de la doctrine i18n.
3. **Supprimer `content/blog/guides/article-modele.mdx` et `content/blog/en/guides/article-model.mdx`** (article de template indexable, hors sitemap, cover 404).
4. **Ajouter un H1 unique sur la home FR et sur `/en`.**
5. **Surcharger la Twitter card par article** (title/description/image) dans les deux routes article.

## Anti-footprint (tech)
Rien à signaler côté technique : le site reprend fidèlement le pattern i18n de référence (`quel-operateur-choisir.be`), ce qui est le comportement attendu, pas un footprint.
